...
__(1) File system structure__: comply with the recommended file system structure

__(2) Config files__: prepare the config files 

__(3) Implement the SAST interface__: python interface of the `tp_framework/core/sast.py` needs to be implemented

__(4) Docker__: create a specific dockerfile and add a service to the docker-compose
...
__(4.1) docker-compose files__: edit the main `docker-compose` files

__(4.2) Dockerfile: create the `Dockerfile` for the SAST tool

__(4.3) Composition/sharing corner-cases__: we experienced some corner-cases situations whose solution can be helpful for you as well
